...
from automation_lib import OUTPUT_DIR, build_context, deploy_configs, deploy_configs_replace, load_yaml, render_configs, validate_intent, write_configs
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--deploy", action="store_true", help="Deploy configs via Netmiko (additive)")
    parser.add_argument("--replace", action="store_true", help="Deploy configs via SCP + Configure Replace (declarative)")
    return parser
...
def main() -> None:
    ...
    if args.replace:
        deploy_configs_replace(context, rendered)

    if not args.write_files and not args.deploy and not args.replace:
